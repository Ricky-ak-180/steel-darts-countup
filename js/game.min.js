...
function renderScrStats() {
  var el = document.getElementById('scr-stats-card');
  if (!el) return;
  var s;
  try { s = JSON.parse(localStorage.getItem('db_scr_stats') || '{}'); } catch(e) { s = {}; }
  var kz1 = s.kz1 || {}, kz2 = s.kz2 || {};
  var today = new Date().toISOString().slice(0, 10);
  var tk1 = (s.today === today && s.todayKz1) ? s.todayKz1 : {};
  var tk2 = (s.today === today && s.todayKz2) ? s.todayKz2 : {};
  var t1 = kz1.total || 0, t2 = kz2.total || 0;
  if (t1 === 0 && t2 === 0) { el.innerHTML = ''; return; }

  function pct(n, t) { return t > 0 ? Math.round(n / t * 100) : 0; }
  function barW(n, t) { return t > 0 ? Math.max(3, Math.round(n / t * 100)) : 0; }
  function ppd(z) { return z.total > 0 ? (Math.round((z.scoreSum || 0) / z.total * 10) / 10).toFixed(1) : '0.0'; }

  function statRow(label, n, t, colorClass) {
    if (t === 0) return '';
    var p = pct(n, t);
    var w = barW(n, t);
    return '<div class="scr-st-row">' +
      '<span class="scr-st-lbl' + (colorClass ? ' ' + colorClass : '') + '">' + label + '</span>' +
      '<div class="scr-st-bar-track"><div class="scr-st-bar' + (colorClass ? ' ' + colorClass : '') + '" style="width:' + w + '%"></div></div>' +
      '<span class="scr-st-pct">' + p + '%</span>' +
      '<span class="scr-st-cnt">(' + n + ')</span>' +
    '</div>';
  }

  function zoneBlock(z, tk, zoneTitle, tri, dbl, single, adj1, adj2, sadj1, sadj2) {
    var total = z.total || 0;
    if (total === 0) return '';
    var onBed = (z[tri] || 0) + (z[dbl] || 0) + (z[single] || 0);
    var successRate = pct(onBed, total);
    var todayTotal = tk.total || 0;
    var todayTri = tk[tri] || 0;
    var todayStr = todayTotal > 0
      ? '<span class="scr-st-today">Today: ' + tri + ' ' + todayTri + '/' + todayTotal + ' (' + pct(todayTri, todayTotal) + '%)</span>'
      : '';
    return '<div class="scr-st-zone">' +
      '<div class="scr-st-zone-hdr">' +
        '<span class="scr-st-zone-name">' + zoneTitle + '</span>' +
        '<span class="scr-st-zone-meta">' + total + '本 | 的中率' + successRate + '% | ' + ppd(z) + 'pt/dart</span>' +
      '</div>' +
      todayStr +
      '<div class="scr-st-rows">' +
        statRow(tri,    z[tri]    || 0, total, 'scr-col-tri') +
        statRow(single, z[single] || 0, total, '') +
        statRow(dbl,    z[dbl]    || 0, total, 'scr-col-dbl') +
        statRow(adj1,   z[adj1]   || 0, total, 'scr-col-tri') +
        statRow(adj2,   z[adj2]   || 0, total, 'scr-col-tri') +
        statRow(sadj1,  z[sadj1]  || 0, total, '') +
        statRow(sadj2,  z[sadj2]  || 0, total, '') +
        statRow('S-BULL', z['S-BULL'] || 0, total, '') +
        statRow('D-BULL', z['D-BULL'] || 0, total, 'scr-col-dbl') +
        statRow('Miss', z['Miss']  || 0, total, 'scr-col-miss') +
      '</div>' +
    '</div>';
  }

  var html = '<div class="scr-st-title">📊 SCRグリッド 入力統計</div>' +
    zoneBlock(kz1, tk1, 'T20エリア', 'T20', 'D20', '20', 'T5', 'T1', '5', '1') +
    zoneBlock(kz2, tk2, 'T19エリア', 'T19', 'D19', '19', 'T7', 'T3', '7', '3');
  el.innerHTML = html;
}
