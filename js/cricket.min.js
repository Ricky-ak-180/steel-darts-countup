...
function _cktRenderHistory() {
  ...
  for (var i = 0; i < h.length; i++) {
    ...
    // Sparkline: up to 10 games ending at this game (chronological order)
    var sparkGames = h.slice(i, i + 10).reverse();
    var sparkHtml = renderCktSparkline(sparkGames, 56, 18);
    ...
    html += '<div style="display:flex;align-items:center;gap:6px;">' +
      (sparkHtml ? '<div style="opacity:0.75;">' + sparkHtml + '</div>' : '') +
      '<div style="font-size:10px;color:var(--mut);">' + ds + '</div>' +
      '</div>';
    html += '</div>';
...
function renderCktStats() {
  ...
  // 統計計算
  var mprs = filtered.map(function(g) { return (Array.isArray(g.mpr) ? g.mpr[0] : g.mpr) || 0; });
  ...
  if (filtered.length >= 2) {
    var latestMpr = (Array.isArray(filtered[0].mpr) ? filtered[0].mpr[0] : filtered[0].mpr) || 0;
    var prevMpr = (Array.isArray(filtered[1].mpr) ? filtered[1].mpr[0] : filtered[1].mpr) || 0;
  ...
  if (weekStats && filtered.length > 1) {
    var recentAvg = (filtered.slice(0, Math.min(3, filtered.length)).reduce(function(sum, g) { return sum + ((g.mpr && g.mpr[0]) || 0); }, 0) / Math.min(3, filtered.length)).toFixed(2);
  ...
  var html = '<div class="ckt-stats-tabs">' + tabs + '</div>' +
    '<div class="ckt-stats-grid">' +
      ...
      '</div>' +
    '</div>' +
    (_cktStatsTab === 'week' ? _renderDailyBarChart() : '');
...
function drawCktChart() {
  ...
  var scores = base.map(function(g, i) {
    ...
    var sum = 0;
    for (var j = 0; j < window10.length; j++) { var m = window10[j].mpr; sum += (Array.isArray(m) ? m[0] : m) || 0; }
...
function renderCktSparkline(games, width, height) {
  ...
  width = width || 60;
  height = height || 20;
  var mprs = games.slice(0, 10).map(function(g) { var m = g.mpr; return (Array.isArray(m) ? m[0] : m) || 0; });
...
function _renderDailyBarChart() {
  ...
  for (var i = 6; i >= 0; i--) {
    ...
    var ds2 = dd.toDateString();
    var dayGames = h.filter(function(g) { return new Date(g.date).toDateString() === ds2; });
    var avgMpr = dayGames.length ? dayGames.reduce(function(s,g){ return s+(g.mpr||0); },0)/dayGames.length : 0;
    days.push({ label: (dd.getMonth()+1)+'/'+(dd.getDate()), mpr: avgMpr, count: dayGames.length });
  }
  var maxMpr = Math.max.apply(null, days.map(function(d){ return d.mpr; })) || 1;
  var W=280, H=90, padL=28, padB=20, padR=4, padT=8;
  var bW = Math.floor((W-padL-padR)/7);
  var cH = H-padT-padB;
  var svg = '<svg width="100%" viewBox="0 0 '+W+' '+H+'" xmlns="http://www.w3.org/2000/svg" style="display:block;">';
  for (var gi=0; gi<=2; gi++) {
    var gy = padT + cH*gi/2;
    var gv = Math.round((maxMpr - maxMpr*gi/2)*100)/100;
    svg += '<line x1="'+padL+'" y1="'+gy+'" x2="'+(W-padR)+'" y2="'+gy+'" stroke="rgba(255,255,255,0.06)" stroke-width="1"/>';
    svg += '<text x="'+(padL-3)+'" y="'+(gy+3)+'" text-anchor="end" font-size="7" fill="rgba(255,255,255,0.3)">'+gv+'</text>';
  }
  for (var bi=0; bi<7; bi++) {
    var day = days[bi];
    var bH = maxMpr>0 ? Math.max(2,(day.mpr/maxMpr)*cH) : 0;
    var bx = padL + bi*bW + 2;
    var by = padT + cH - bH;
    var isToday = (bi===6);
    if (day.count > 0) {
      svg += '<rect x="'+bx+'" y="'+by+'" width="'+(bW-4)+'" height="'+bH+'" fill="'+(isToday?'#47ffb4':'rgba(71,255,180,0.45)')+'" rx="2"/>';
      if (bH > 14) svg += '<text x="'+(bx+(bW-4)/2)+'" y="'+(by-2)+'" text-anchor="middle" font-size="7" fill="rgba(71,255,180,0.9)">'+day.mpr.toFixed(1)+'</text>';
    } else {
      svg += '<rect x="'+bx+'" y="'+(padT+cH-2)+'" width="'+(bW-4)+'" height="2" fill="rgba(255,255,255,0.06)" rx="1"/>';
    }
    svg += '<text x="'+(bx+(bW-4)/2)+'" y="'+(H-3)+'" text-anchor="middle" font-size="7" fill="rgba(255,255,255,'+(isToday?'0.7':'0.3')+')">'+day.label+'</text>';
  }
  svg += '</svg>';
  return '<div style="margin-top:8px;padding:8px;background:rgba(255,255,255,0.03);border-radius:6px;">'+
    '<div style="font-size:9px;color:var(--mut);letter-spacing:1px;margin-bottom:4px;">7-DAY MPR TREND</div>'+
    svg+'</div>';
}
